...
private:
    ...
    static void thomson_draw(TMultiGraph *mg, const SignalProcessing &sp, uint nPoints, const int integrate, bool draw=true, bool drawSigBox=false, const std::vector<TString> &gTitle={}, const barray &work_mask={});
...
public:
    ...
    static void draw_result_from_r(TCanvas *c, TMultiGraph *mg, const darray &xPosition, const darray &result, const darray &result_error, uint marker_style=kFullSquare, float marker_size=1.5, uint markerColor=1, uint lineWidth=1, uint lineStyle=10, bool draw=true);
